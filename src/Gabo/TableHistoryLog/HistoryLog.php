...
namespace Gabo\TableHistoryLog;
...
trait HistoryLog
{
     ...
     *
     */
    public static function bootHistoryLog()
    {
    ...
    {
        return $this->morphMany('\Gabo\TableHistoryLog\History', 'Historical');
    }
    ...
    {
        return History::where('history_type', get_called_class())
    ...
    public function postSave()
    {
        ...
        if (((!isset($this->revisionEnabled) || $this->revisionEnabled) && $this->updating) && (!$LimitReached || $RevisionCleanup)) {
            ...
            foreach ($changes_to_record as $key => $change) {
                $histories[] = array(
                    'history_type' => $this->getMorphClass(),
            ...
            if (count($histories) > 0) {
                if($LimitReached && $RevisionCleanup){
                    $toDelete = $this->revisionHistory()->orderBy('id','asc')->limit(count($histories))->get();
                ...
                $history = new History();
                 ...
                 \DB::table($history->getTable())->insert($histories);


                \Event::fire('historical.saved', array('model' => $this, 'revisions' => $histories));
            }
        }
    }
